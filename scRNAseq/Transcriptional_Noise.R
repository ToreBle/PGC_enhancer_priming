...
States <- c("ESC", "d1 EpiLC", "d2 EpiLC", "d3 EpiLC", "EpiSC")
noise <- data.frame()
for(i in States){
  print(i)
...
noise$State <- factor(noise$State, levels=c("ESC", "d1 EpiLC", "d2 EpiLC", "d3 EpiLC", "EpiSC"))
...
ggplot(noise, aes(State, cor, fill=State))+geom_violin()+geom_boxplot(width=0.3, outlier.shape = NA, fill="white")+
  ...
  theme_classic()+scale_fill_manual(values = c("#0101DF", "#8904B1", "#DF0101", "#B40404", "#FF8000"))+
